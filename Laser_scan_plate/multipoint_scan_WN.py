...
def analog_out_noise(analogOut, channel=0, amplitude=3.0, bandwidth_hz=10000.0):
    ...
    # 1. Remise à zéro du canal
    analogOut.reset(channel)
    
    # 2. Activation du canal
    analogOut.nodeEnableSet(channel, node, True)
    
    # 3. SÉLECTION DU BRUIT ALÉATOIRE MATÉRIEL
    analogOut.nodeFunctionSet(channel, node, DwfAnalogOutFunction.Noise)
    
    # 4. Taux de rafraîchissement (Bande passante couverte par le bruit)
    analogOut.nodeFrequencySet(channel, node, bandwidth_hz) 
    
    # 5. Réglage de la puissance (Tension)
    analogOut.nodeAmplitudeSet(channel, node, amplitude)
    analogOut.nodeOffsetSet(channel, node, 0.0)
    
    # 6. Configuration pour une exécution INFINIE (0.0)
    analogOut.runSet(channel, 0.0)
    analogOut.repeatSet(channel, 0)
    
    # 7. Application immédiate et démarrage
...
def acquisition(analogIn, sample_frequency, record_length):
    ...
    analogIn.triggerSourceSet(DwfTriggerSource.None_)
